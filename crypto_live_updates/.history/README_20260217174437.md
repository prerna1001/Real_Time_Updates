...
A **real-time cryptocurrency price tracking application** that combines a scalable Node.js WebSocket backend with a modern Next.js frontend. Built with TypeScript and organized as a monorepo using pnpm workspaces.
...
- **[EventEmitter3](https://github.com/primus/eventemitter3)** - Event-driven architecture
...
5. **WebSocket Client**: Automatic connection to backend WebSocket for live updates
...
- Establishes a WebSocket connection to the backend on component mount
...
Price updates are transmitted as JSON objects with the following structure:
...
1. **Client connects**: Frontend establishes WebSocket connection to `ws://localhost:4000/prices`
2. **Server accepts**: Backend accepts the connection and adds client to broadcast list
3. **Events flow**: Backend broadcasts all price events to connected clients
4. **Client receives**: Frontend listens for messages and updates the event log
5. **Disconnection**: Frontend automatically closes connection on unmount
...
```env
NEXT_PUBLIC_WEBSOCKET_URL=ws://localhost:4000/prices
```
...
- **Mocked Prices**: The TradingView scraper currently generates random prices instead of real market data
- **No Persistence**: Price data is not stored in a database (only maintained in EventEmitter memory)
- **Single Browser Instance**: Only one Playwright browser instance is active at a time
- **No Authentication**: No user authentication or authorization system
- **Limited Error Handling**: Basic error handling for WebSocket connections

### Future Enhancements

- [ ] Real TradingView DOM scraping with proper selectors
- [ ] Database integration for historical price data
- [ ] Multi-symbol real-time price aggregation
